...
func TestDefaultPinsFallsBackToFixedBoardPinsWhenDiscoveryIsEmpty(t *testing.T) {
	restoreListExternalPins(t, nil)
	...
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("DefaultPins() = %#v, want %#v", got, want)
	}
}

func TestDefaultPinsUsesDiscoveredPinsWhenAvailable(t *testing.T) {
	restoreListExternalPins(t, []int{3, 1})
...
func restoreListExternalPins(t *testing.T, numbers []int) {
	t.Helper()

	original := listExternalPins
	listExternalPins = func() []int {
		return append([]int(nil), numbers...)
	}
	t.Cleanup(func() {
		listExternalPins = original
	})
}
