...
func (d *Decoder) Decode(ctx context.Context, packet model.DIDEncrypted, deviceSN string, receivedAt time.Time) Output {
...
func (d *Decoder) decodedOutput(packet model.DIDEncrypted, result DecryptResult, receivedAt time.Time) Output {
...
func (d *Decoder) uncrackedOutput(packet model.DIDEncrypted, receivedAt time.Time, status Status, err error) Output {
...
func TargetFromDecryptResult(packet model.DIDEncrypted, result DecryptResult, receivedAt time.Time, cracked bool) model.PositionTarget {
	...
	return model.PositionTarget{
		CorrelationID:    correlationID(packet.EncryptedID),
		Serial:           serial,
		Model:            modelName,
		Source:           model.TypeDIDEncrypted,
...
func (d *Decoder) cacheKeyPacket(encryptedID, hexStr string, packet model.DIDEncrypted) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func pointFromLatLng(lat, lng float64) *model.Point {
	if !validCoordinate(lat, lng) {
		return nil
	}
	return &model.Point{Latitude: lat, Longitude: lng}
}
