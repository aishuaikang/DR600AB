// Package interferencereport persists interference operation evidence reports.
...
`
	if _, err := s.db.Exec(schema); err != nil {
		return fmt.Errorf("初始化干扰报告库失败: %w", err)
	}
	return nil
}
...
func (s *Store) Update(report model.InterferenceReport) error {
	...
	if err != nil {
		return fmt.Errorf("更新干扰报告失败: %w", err)
	}
	affected, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("读取干扰报告更新数量失败: %w", err)
	}
	if affected == 0 {
		return ErrNotFound
	}
...
func (s *Store) insert(report model.InterferenceReport) error {
	...
	if err != nil {
		return fmt.Errorf("写入干扰报告失败: %w", err)
	}
	return nil
}

// List returns report summaries ordered by newest start time first.
func (s *Store) List(options QueryOptions) ([]model.InterferenceReportSummary, error) {
	if s == nil || s.db == nil {
		return nil, nil
	}
	limit := options.Limit
	...
	if err != nil {
		return nil, fmt.Errorf("查询干扰报告失败: %w", err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("读取干扰报告失败: %w", err)
	}
	return items, nil
}

// Get returns a full interference report by ID.
func (s *Store) Get(id string) (model.InterferenceReport, error) {
	if s == nil || s.db == nil {
...
func (s *Store) DeleteFailed(id string) (int64, error) {
	...
	var status string
	if err := s.db.QueryRow(`SELECT status FROM interference_reports WHERE id = ?`, id).Scan(&status); err != nil {
	...
	result, err := s.db.Exec(`DELETE FROM interference_reports WHERE id = ? AND status = ?`, id, string(model.InterferenceReportStatusFailed))
	if err != nil {
		return 0, fmt.Errorf("删除干扰报告失败: %w", err)
	}
	deleted, err := result.RowsAffected()
	if err != nil {
		return 0, fmt.Errorf("读取干扰报告删除数量失败: %w", err)
	}
	return deleted, nil
}
...
func (s *Store) CloseRunning(reason string, now time.Time) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("闭合运行中干扰报告失败: %w", err)
	}
	affected, err := result.RowsAffected()
	if err != nil {
		return 0, fmt.Errorf("读取闭合干扰报告数量失败: %w", err)
	}
	return affected, nil
}
...
func scanSummary(rows *sql.Rows) (model.InterferenceReportSummary, error) {
	...
	if err != nil {
		return model.InterferenceReportSummary{}, fmt.Errorf("解析干扰报告摘要失败: %w", err)
	}
	summary.LastError = normalizeReportError(summary.LastError)
...
func scanReport(scanner rowScanner) (model.InterferenceReport, error) {
	...
	if err != nil {
		...
		return model.InterferenceReport{}, fmt.Errorf("解析干扰报告失败: %w", err)
	}
	report.LastError = normalizeReportError(report.LastError)
...
func normalizeReportSummary(report model.InterferenceReport) model.InterferenceReport {
	...
	if len(report.ChannelIDs) == 0 {
		report.ChannelIDs = append([]string(nil), report.Request.ChannelIDs...)
	}
	if report.StartState != nil {
		...
		if len(report.ChannelIDs) == 0 {
			report.ChannelIDs = append([]string(nil), report.StartState.ChannelIDs...)
		}
...
func normalizeReportError(message string) string {
...
func ParseStatus(value string) (model.InterferenceReportStatus, error) {
	...
	case "", model.InterferenceReportStatusRunning, model.InterferenceReportStatusCompleted, model.InterferenceReportStatusFailed, model.InterferenceReportStatusAbnormal:
		return status, nil
	default:
