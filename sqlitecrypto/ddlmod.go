...
func parseDDL(strs ...string) (*ddl, error) {
	...
	for _, str := range strs {
		if sections := tableRegexp.FindStringSubmatch(str); len(sections) > 0 {
			...
			// Do not report Unique by UniqueIndex.
		} else {
...
func (d *ddl) removeColumn(name string) bool {
	for i := 0; i < len(d.fields); i++ {
		if matches := columnRegexp.FindStringSubmatch(d.fields[i]); len(matches) > 1 && matches[1] == name {
			d.fields = append(d.fields[:i], d.fields[i+1:]...)
			return true
		}
	}
...
func (d *ddl) getColumns() []string {
	...
	for _, f := range d.fields {
		...
		if matches := columnRegexp.FindStringSubmatch(f); len(matches) > 1 {
...
func parseAllColumns(s string) ([]string, error) {
	start := strings.IndexByte(s, '(')
	end := strings.LastIndexByte(s, ')')
	if start == -1 || end == -1 || end <= start {
		return nil, fmt.Errorf("invalid column list: %s", s)
	}
	...
	for _, c := range body {
		if sc := string(c); separatorRegexp.MatchString(sc) {
			...
			if c == '(' {
				...
				if col := normalizeColumnName(buf); col != "" {
	...
	if bracketLevel != 0 {
		return nil, errors.New("invalid column list, unbalanced brackets")
	}
	if col := normalizeColumnName(buf); col != "" {
		columns = append(columns, col)
	}
